...
static void UseAfterFreeTest(void *pvParameters) {
  ...
  t_out = xTaskGetTickCount();
  emit_line("TIME,uaf,memset,%u,%lu,%lu,DONE,%u,%u\r\n", BLOCK_SIZE,
            (unsigned long)t_in, (unsigned long)t_out, alloc_cnt, free_cnt);
  t_in = xTaskGetTickCount();
  p2 = pvPortMalloc(BLOCK_SIZE);
  t_out = xTaskGetTickCount();
  if (p2 == NULL) {
    emit_line("TIME,alloc2,malloc,%u,%lu,%lu,NULL,%u,%u\r\n", BLOCK_SIZE,
              (unsigned long)t_in, (unsigned long)t_out, alloc_cnt, free_cnt);
    emit_line("FAULT,%lu,0xDEAD,OOM\r\n", (unsigned long)xTaskGetTickCount());
    goto done;
  }
  alloc_cnt++;
  emit_line("TIME,alloc2,malloc,%u,%lu,%lu,OK,%u,%u\r\n", BLOCK_SIZE,
            (unsigned long)t_in, (unsigned long)t_out, alloc_cnt, free_cnt);
  ...
  buf2 = (uint8_t *)p2;
  {
    ...
    if (leaked) {
      emit_line("TIME,inspect,check,%u,%lu,%lu,LEAKED,%u,%u\r\n", BLOCK_SIZE,
                (unsigned long)xTaskGetTickCount(),
                (unsigned long)xTaskGetTickCount(), alloc_cnt, free_cnt);
    } else {
      emit_line("TIME,inspect,check,%u,%lu,%lu,CLEAN,%u,%u\r\n", BLOCK_SIZE,
                (unsigned long)xTaskGetTickCount(),
                (unsigned long)xTaskGetTickCount(), alloc_cnt, free_cnt);
    }
  }
...
int main(void) {
  Board_init();
  xTaskCreate(UseAfterFreeTest, "UseAfterFreeTest", 512, NULL, 1, NULL);
  vTaskStartScheduler();
  for (;;)
    ;
  return 0;
